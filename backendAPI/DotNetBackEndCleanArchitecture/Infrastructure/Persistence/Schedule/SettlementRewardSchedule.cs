...
using DomainEntity.ScheduleEntity.SettlementRewardSchedule;
...
{
    public class SettlementRewardSchedule
    {
        private readonly ILogger _logger;
        private readonly IDBAccess _dbAccess;
        private readonly string _strConnection;

        private readonly string pBaseLog = "SettlementRewardSchedule";

        public SettlementRewardSchedule(
                ILogger<SettlementRewardSchedule> logger,
                IDBAccess dbAccess,
        ...
        public ServiceResultDTO<bool> Process()
        {
            ...
            try
            {
                string mSqlGetSettlementRewardData = @"
SELECT A.tr_id
       ,A.tr_code
       ,A.tr_mm_id
       ,A.tr_tm_id
...
";
                ...
                List<SettlementRewardPO> mGetTrData = _dbAccess.ExecuteTransactionGetObject<SettlementRewardPO>(mSqlGetSettlementRewardData, null).ToList();
                _logger.LogInformation($@"{pBaseLog} {mActionLog} {mGetTrData.Count}");
                List<member_balance> mInsertBalance = new List<member_balance>();
                Dictionary<long, SettlementRewardInsertBalancePoint> mOriginalWallet = new Dictionary<long, SettlementRewardInsertBalancePoint>();
                ...
                foreach (var item in mGetTrData)
                {
                    ...
                    {
                        mOriginalWallet.Add(item.tr_mm_id, new SettlementRewardInsertBalancePoint { mw_stored = item.mw_stored, mw_reward = item.mw_reward, mw_mall = item.mw_mall });
                        mMWCnt++;
                    }

                    SettlementRewardInsertBalancePoint originalWalletItem = mOriginalWallet[item.tr_mm_id];

                    decimal mStored = item.tr_mm_points * 0.9m;
                    decimal mMall = item.tr_mm_points * 0.1m;

                    originalWalletItem.mw_reward -= item.tr_mm_points;
                    originalWalletItem.mw_stored += mStored;
                    originalWalletItem.mw_mall += mMall;
                    originalWalletItem.mw_reward_chenge += item.tr_mm_points;
                    originalWalletItem.mw_stored_chenge += mStored;
                    originalWalletItem.mw_mall_change += mMall;
                    ...
                    if (item.tr_type == "13")
                    {
                        mInsertBalance.Add(new member_balance()
                        {
                            ...
                            mb_type = "14",
                            mb_points_type = "12",//紅利點數
                            mb_change_points = item.tr_mm_points,
                            mb_points = originalWalletItem.mw_reward,
                ...
                var mUpdateWallet = mOriginalWallet.Select(kv => new
                {
                    ...
                    mw_reward = kv.Value.mw_reward_chenge,
                    mw_mall = kv.Value.mw_mall_change,
                    mw_update_member = kv.Key,
                    mw_update_datetime = mNowDateTime,
                    mw_mm_id = kv.Key
                }).ToList();

                mIsOk = _dbAccess.ExecuteTransactionObject<List<member_balance>>(mSqlInsertBalance, mInsertBalance) == (mCheckCnt * 3);
                if (!mIsOk)
                {
                    _dbAccess.Rollback();

                    mResult.returnStatus = 999;
                    mResult.returnMsg = "紅利排程失敗、新增帳務紀錄筆數錯誤";
                    return mResult;
                }
                ...
                if (!mIsOk)
                {
                    ...
                    mResult.returnStatus = 999;
                    mResult.returnMsg = "紅利排程失敗、更新錢包筆數錯誤";
                    return mResult;
                }

                mSqlUpdateTR = string.Format(mSqlUpdateTR, string.Join(',', mGetTrData.Select(s => s.tr_id)));
                mIsOk = _dbAccess.ExecuteTransactionObject<object>(mSqlUpdateTR, new { tr_update_datetime = mNowDateTime }) == mCheckCnt;
                if (!mIsOk)
                {
                    _dbAccess.Rollback();

                    mResult.returnStatus = 999;
                    mResult.returnMsg = "紅利排程失敗、更新交易紀錄筆數錯誤";
                    return mResult;
                }
                else
                {
