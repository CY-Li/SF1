...
import { IQueryWalletReqModel, IQueryWalletRespModel } from './wallet.model';
import { WalletService } from './wallet.service';
import { WalletAddEditComponent } from './wallet-add-edit/wallet-add-edit.component';
...
export class WalletComponent implements AfterViewInit {
  ...
  // 傳送給伺服器接收參數
  formData: IQueryWalletRespModel = {};
  // 定義頁面Form表單用
  formGroup: FormGroup;
  // 初始化Form表單條件用
  initFormGroup() {
  ...
  totalpages: number = 0; // 頁面分頁頁數數入框最大輸入值
  currnPageIndex: number = 0; // 頁面分頁頁數數入框目前頁數顯示內容
  // Table資料放置位置，並將欄位定義Interface匯入
  dataSource: MatTableDataSource<IQueryWalletRespModel> = new MatTableDataSource();
  ...
  queryFrontPageList(pageIndex: number, pageSize: number, sortColumn: string, sortDirection: string) {
    ...
    if (this.formGroup.valid) {
      const formData: IQueryWalletReqModel = this.formGroup.getRawValue();
      ...
      this._service.QueryWallet(formData).subscribe({
        next: (data) => {
          console.log(data);
  ...
  openAddEditForm(isView: boolean, formData?: any,) {
    const dialogRef = this._dialog.open(WalletAddEditComponent, {
