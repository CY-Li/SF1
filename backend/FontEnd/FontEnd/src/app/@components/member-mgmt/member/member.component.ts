...
import { IMember, IQueryMember } from './member.model';
...
export class MemberComponent implements AfterViewInit {
  ...
  initFormGroup() {
    ...
    return this.formGroup = this._formBuilder.group({
      ...
      // mm_introduce_code: this._formBuilder.control('', Validators.pattern('^[0-9]*$')),
      mm_introduce_user: this._formBuilder.control('', Validators.pattern('^[0-9]*$')),
      // mm_invite_code: this._formBuilder.control('', Validators.pattern('^[0-9]*$'))
      mm_invite_user: this._formBuilder.control('', Validators.pattern('^[0-9]*$'))
    })
  }
  ...
  constructor(
    ...
    private _service: MemberService // 後端API服務呼叫用
  ...
  queryFrontPageList(pageIndex: number, pageSize: number, sortColumn: string, sortDirection: string) {
    ...
    if (this.formGroup.valid) {
      const formData: IQueryMember = this.formGroup.getRawValue();
      ...
      this._service.QueryMember(formData).subscribe({
        next: (data) => {
          console.log(data);
          if (data && data.result && data.returnStatus == 1) {
            ...
            this.currnPageIndex = pageIndex + 1;
  ...
  onPageInputChange() {
    this.currnPageIndex = Math.max(1, Math.min(this.currnPageIndex, this.totalpages));
    // this.paginator.pageIndex = this.currnPageIndex - 1;
    this.queryFrontPageList(this.currnPageIndex - 1, this.pageSize, this.pageSort, this.pageSortDirection);
  }
