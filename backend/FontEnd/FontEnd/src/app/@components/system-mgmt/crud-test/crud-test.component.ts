...
import { CrudTestAddEditComponent } from './crud-test-add-edit/crud-test-add-edit.component';
...
import { IQuerySpsReq, IQuerySpsRespModel } from '../sps/sps.model';
...
@Component({
  ...
  styleUrl: './crud-test.component.scss'
})
export class CrudTestComponent {
    ...
    merge(this.sort.sortChange, this.paginator.page)
      .pipe(
        tap(() => {
          ...
          this.querySpsList(this.paginator.pageIndex, this.paginator.pageSize, this.sort.active, this.sort.direction);
        })
      ).subscribe(); // 頁面分頁/排序訂閱，監視分頁/排序事件

    // this.paginator.page.subscribe((page: PageEvent) => {
    //   this.querySpsList(page.pageIndex + 1, page.pageSize);
    // }); // 頁面分頁訂閱，監視分頁事件
  ...
  // 查詢頁面顯示內容
  querySpsList(pageIndex: number, pageSize: number, sortColumn: string, sortDirection: string) {
    ...
    if (this.formGroup.valid) {
      ...
      this._spsService.querySpsList(formData).subscribe({
        next: (data) => {
          console.log(data);
  ...
  openAddEditForm(isView: boolean, formData?: any,) {
    const dialogRef = this._dialog.open(CrudTestAddEditComponent, {
    ...
    dialogRef.afterClosed().subscribe({
      next: (val) => {
        ...
        if (val) {
          this.querySpsList(0, this.pageSize, this.pageSort, this.pageSortDirection);
        }
      },
    });
  }
  ...
  onDelete(id: number, message: string) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        // this._spsService.querySpsList(id).subscribe({
        //   next: (data) => {
        //     if (data && data.result && data.returnStatus == 1) {
