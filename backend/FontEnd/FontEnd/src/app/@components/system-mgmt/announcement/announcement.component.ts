...
export class AnnouncementComponent {
  ...
  constructor(
    private _announcementService: AnnouncementService,//後端API
  ...
  // 查詢頁面顯示內容
  getAnnouncement() {
    this._announcementService.getAnnouncementImages().subscribe({
      next: (data) => {
        console.log(data);
  ...
  onSubmit() {
    ...
    // 送出表單到 API
    this._announcementService.putAnnouncementImages(formData).subscribe({
      next: (data) => {
        console.log(data);
        if (data && data.result && data.returnStatus == 1) {
          alert("成功");
